...
===============================================================================
  ...
  FILE:  laspointreader3compressedarithmetic.cpp
...
bool LASpointReader3compressedArithmetic::read_point(LASpoint* point, double* gps_time, unsigned short* rgb)
{
	if (dec)
	{
		*point = last_point;
    ...
    // decompress the rgb color ... if it has changed
		if (changed_values & 128)
		{
			...
			rgb[1] = ic_r->Decompress(last_rgb[1]);
			rgb[2] = ic_r->Decompress(last_rgb[2]);
		}
...
LASpointReader3compressedArithmetic::LASpointReader3compressedArithmetic(FILE* file)
{
	this->file = file;
  last_dir = 0;
...
void LASpointReader3compressedArithmetic::init_decoder()
{
...
LASpointReader3compressedArithmetic::~LASpointReader3compressedArithmetic()
{
	if (dec)
	{
		dec->done();
