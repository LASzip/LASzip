...
===============================================================================
  ...
  FILE:  laspointreader3compressed.cpp
...
bool LASpointReader3compressed::read_point(LASpoint* point, double* gps_time, unsigned short* rgb)
{
	if (rd)
	{
		*point = last_point;
    ...
    else
    {
      y_diff = ic_dy->DecompressLast(median_y, (last_dir?messy+7:messy));
		...
		{
			int multi = rd->decode(rm_gps_time_multi[messy==2]);
        ...
        if (multi)
          ...
          gps_time_diff = ic_gps_time->DecompressNone();
				...
				((__int64*)gps_time)[0] = ((__int64*)&(last_gps_time))[0] + gps_time_diff;
        ...
        if (multi == 1 || multi_extreme_counter > 3)
			...
			{
				*gps_time = rd->readDouble();
			}
    }

    // decompress the rgb color ... if it has changed
		if (changed_values & 128)
		{
			rgb[0] = ic_r->DecompressLast(last_rgb[0]);
			rgb[1] = ic_r->DecompressLast(last_rgb[1]);
			rgb[2] = ic_r->DecompressLast(last_rgb[2]);
		}
...
LASpointReader3compressed::LASpointReader3compressed(FILE* file)
{
	this->file = file;
  last_dir = 0;
...
void LASpointReader3compressed::init_decoder()
{
  ...
  ic_dy->SetupDecompressor(rd, 14);
  ic_z->SetupDecompressor(rd, 7);
  ...
  ic_gps_time->SetupDecompressor(rd);
	...
	ic_r->SetupDecompressor(rd); 
	ic_g->SetupDecompressor(rd); 
	ic_b->SetupDecompressor(rd); 
}

LASpointReader3compressed::~LASpointReader3compressed()
{
	if (rd)
	{
		rd->done();
