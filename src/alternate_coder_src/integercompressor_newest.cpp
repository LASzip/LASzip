...
===============================================================================
  ...
  FILE:  integercompressor_newest.cpp
...
void IntegerCompressorContext::SetupCompressor(ArithmeticEncoder* enc, int contexts, int bits_high)
{
  int i;
  ...
  mBitsNone = new ArithmeticModel(corr_bits+1,0,1);
  mBitsLast = (ArithmeticModel**)malloc(sizeof(ArithmeticModel*) * (contexts));
  for (i = 0; i < contexts; i++)
  {
    mBitsLast[i] = new ArithmeticModel(corr_bits+1,0,1);
  }
...
{
  int i;
  if (mBitsNone) {delete mBitsNone; mBitsNone = 0;}
  if (mBitsLast)
  {
    for (i = 0; i < contexts; i++)
    {
...
void IntegerCompressorContext::SetupDecompressor(ArithmeticDecoder* dec, int contexts, int bits_high)
{
  int i;
  ...
  mBitsNone = new ArithmeticModel(corr_bits+1,0,0);
  mBitsLast = (ArithmeticModel**)malloc(sizeof(ArithmeticModel*) * (contexts));
  for (i = 0; i < contexts; i++)
  {
    mBitsLast[i] = new ArithmeticModel(corr_bits+1,0,0);
  }
...
// writeCorrector:
//-----------------------------------------------------------------------------
I32 IntegerCompressorContext::writeCorrector(I32 c, ArithmeticModel* mBits)
{
  I32 c1, k;
...
I32 IntegerCompressorContext::CompressNone(I32 real)
{
  ...
  last = real;
  return writeCorrector(corr, mBitsNone);
}
...
I32 IntegerCompressorContext::CompressLast(I32 pred, I32 real, int context)
{
  ...
  last = real;
  return writeCorrector(corr, mBitsLast[context]);
}

I32 IntegerCompressorContext::readCorrector(ArithmeticModel* mBits)
{
  I32 c;

  // decode within which interval the corrector is falling

  int k = dec->decode(mBits);
...
{
  I32 real = last + readCorrector(mBitsNone);
...
{
  I32 real = pred + readCorrector(mBitsLast[context]);
  if (real < 0) real += corr_range;
  else if (real >= corr_range) real -= corr_range;
  last = real;
  return real;
