        ...
        if (compatibility_VLR)
        {
          ...
          if ((laszip_dll->start_scan_angle != -1) && (laszip_dll->start_extended_returns != -1) && (laszip_dll->start_classification != -1) && (laszip_dll->start_flags_and_channel != -1))
          {
            ...
            for (U32 i = 0; i < 15; i++)
            {
              ...
              {
#ifdef _WIN32
                fprintf(stderr,"WARNING: number_of_points_by_return[%u] is %u. but extended_number_of_points_by_return[%u] is %I64d.\n", i, laszip_dll->header.number_of_points_by_return[i], i, extended_number_of_points_by_return);
#else
...
)
{
  ...
  if (0) // (laszip_dll->lax_exploit) // maybe not possible via your stream interface?
  {
    ...
    if (!laszip_dll->lax_index->read(laszip_dll->file)) // laszip_dll->file points to the wrong file. would have broken my DLL.
    {
      delete laszip_dll->lax_index;
      laszip_dll->lax_index = 0;
    }
...
)
{
  ...
  try
  {
    ...
    return laszip_reader_init(laszip_dll, is_compressed);
  }
  catch (...)
  {
    sprintf(laszip_dll->error, "internal error in laszip_open_reader");
...
)
{
  ...
  try
  {
    ...
    create_point_writer(laszip_dll, &laszip);
  }
  catch (...)
  {
